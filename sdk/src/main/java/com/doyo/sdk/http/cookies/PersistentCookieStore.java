package com.doyo.sdk.http.cookies;
...
public class PersistentCookieStore {
    ...
    private static final String                                         COOKIE_PREFS =
            "Cookies_Prefs";
    private final        Map<String, ConcurrentHashMap<String, Cookie>> cookies;
    private final        SharedPreferences                              cookiePrefs;

    PersistentCookieStore() {
        cookiePrefs = GlobalApplication.getInstance().getSharedPreferences(COOKIE_PREFS, 0);
        cookies = new HashMap<>();
    ...
    boolean remove(HttpUrl url, Cookie cookie) {
        ...
        if (cookies.containsKey(url.host()) && cookies.get(url.host()).containsKey(name)) {
            ...
            prefsWriter.putString(url.host(), TextUtils.join(",",
                    cookies.get(url.host()).keySet()));
            prefsWriter.apply();
